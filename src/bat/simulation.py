        ...
        while collected < steps:
            ...
            if on_log:
                on_log(
                    ...
                    f"信心={decision.confidence:.2%} 預期={expected_return:.2%} 實際={actual_return:.2%} "
                    f"止損={risk.stop_loss:.2% if risk else 'N/A'} 止盈={risk.take_profit:.2% if risk else 'N/A'} "
                    f"最大回撤={risk.max_dd_stop:.2% if risk else 'N/A'} 分段={risk.position_splits if risk else 'N/A'}"
                )
